...
% $JRev: R-g V:006 Sum:gyTx+hhaVw10 Date:09-Oct-2011 02:42:27 $
...
whichWindowAPI = which('WindowAPI');
if isempty(whichWindowAPI)
   error(['JSimon:', mfilename, ':MissingFunction'], ...
      ['*** %s: WindowAPI is not found in the path.\n', ...
      '    Copy the function to a folder of your MATLAB path.'], mfilename);
end

[fPath, fName, fExt] = fileparts(whichWindowAPI);  %#ok<ASGLU>
if strcmpi(fExt, ['.', mexext]) == 0
   if isempty(which(['WindowAPI.', mexext]))
      error(['JSimon:', mfilename, ':MissingMex'], ...
         ['*** %s: The compiled Mex function is missing.\n', ...
         '  Please run WindowAPI without inputs for an auto-compilation\n', ...
         '  or read the COMPILE section in WindowAPI.c'], mfilename);
   else
      which_all = cellstr(which('WindowAPI', '-all'));
      fprintf(2, 'Shadowed Mex function:\n');
      fprintf('  %s\n', which_all{:});
      error(['JSimon:', mfilename, ':ShadowedMex'], ...
         ['*** %s: The compiled Mex function is shadowed.\n', ...
         '  Please use CD to change to a different folder.'], mfilename);
   end
end
...
try
   ...
   h   = figure('Color', ones(1, 3), 'Renderer', 'Painters');
   FigH = h.Number;
   ...
   WindowAPI(FigH, 'ToScreen');  % If 2nd monitor has different size
   pause(delay);
   ...
   % Set topmost status:
   disp('  Set figure to topmost, notopmost and to front:');
